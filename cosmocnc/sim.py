...
#     # after creating cpdf_samples
#     cpdf_samples = np.random.rand(n_samples)
#     cpdf_samples = np.clip(cpdf_samples, z[0], 1.0)

#     # interpolator = interpolate.RegularGridInterpolator((z,y),x_matrix,method='linear',bounds_error=True)
#     interpolator = interpolate.RegularGridInterpolator((z,y), x_matrix, method='linear', bounds_error=False, fill_value=x[0])
#     x_samples = interpolator((cpdf_samples,y_samples))
